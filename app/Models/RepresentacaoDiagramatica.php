...
class RepresentacaoDiagramatica extends Model
{
    ...
    protected $table = 'representacoes_diagramaticas';
    protected $fillable = [
        'nome',
        'descricao',
        'xml_modelo',
        'svg_modelo',
        'xml_modelo_comentado',
        'validado',
        'codprojeto',
        'codrepositorio',
        'codusuario',
    ...
    public function existeSVG(){
        return !empty($this->svg_modelo);
    }
    public function configuracoesAmbienteModelagem(){
        return $this->hasMany(ConfiguracaoAmbienteModelagem::class,'codmodelodiagramatico','codmodelodiagramatico');
    }
    public function existeConfiguracaoAmbienteModelagem(){
        foreach ($this->configuracoesAmbienteModelagem as $configuracao){
            if($configuracao->codusuario === Auth::getUser()->codusuario){
                return true;
            }
        }
        return false;
    }
    public function getConfiguracaoAmbienteModelagem(){
        foreach ($this->configuracoesAmbienteModelagem as $configuracao){
            if($configuracao->codusuario === Auth::getUser()->codusuario){
                return $configuracao;
            }
    ...
    {
        return $this->modelo->UsuarioTemPermissao($user);
    }
    public function permissao(){
        if($this->codusuario === Auth::getUser()->codusuario) return true;
        return $this->modelo->permissao();
    }
//Instancia todas as posições de memória que serão exibidas no título
    ...
    public function historico_alteracoes()
    {
        return $this->hasMany(RepresentacaoDiagramaticaVersionavel::class, 'codmodelodiagramatico', 'codmodelodiagramatico');
    }
...
//Relacionamentos
    ...
    public function participantes(){
        $cod_usuarios = collect([]);
        foreach ($this->modelo->usuarios_modelos as $usuario_modelo){
            $cod_usuarios->add($usuario_modelo->codusuario);
        }
        $cod_usuarios = $cod_usuarios->unique();
        $participates = collect([]);
        foreach ($cod_usuarios as $cod_usuario){
            if (Auth::getUser()->codusuario !== $cod_usuario){
                $participates->add(User::Find($cod_usuario));
            }
        }
        return $participates;
    }
    public function diagramaEstaComentado(){
        return !empty($this->xml_modelo_comentado);
    }
    ...
    public function eColaborador(){
        return $this->modelo->Ecolaborador() || $this->modelo->Eproprietario();
    }

    public function eProprietario(){
        return $this->modelo->Eproprietario();
    }

    public function eCliente(){
        return $this->modelo->Ecliente() || $this->modelo->Eproprietario() || $this->modelo->Ecolaborador();
    }
    ...
    {
        return $this->hasOne(User::class, 'codusuario', 'codusuariovalidador');
    }
    ...
    {
        return $this->hasMany(Arquivo::class, 'codmodelodiagramatico', 'codmodelodiagramatico');
    }

    public function todos_usuarios()
    {
        return Cache::remember('todos_usuarios' . $this->codmodelodiagramatico, 2000, function () {
            $usuarios = [];
    ...
    protected static function boot()
    {
        ...
        static::updated(function ($modelo) {
            try {
                ...
                $dado = [
                    ...
                    'xml_modelo' => $modelo->xml_modelo ? $modelo->xml_modelo : $this->get_modelo_default1(),
                    'svg_modelo' =>$modelo->svg_modelo ? $modelo->svg_modelo : $this->svgPadrao(),
                ...
                RepresentacaoDiagramaticaVersionavel::create($dado);
                DB::commit();
            } catch (\Exception $ex) {
                DB::rollBack();
            }
            AcessoRecenteRepository::CriaAcessoRecenteDiagrama($modelo, 'edicao_diagrama', 'Atualização do diagrama ' . $modelo->nome);
            limpar_cache();
        });
        ...
        static::created(function ($diagrama) {
            AcessoRecenteRepository::CriaAcessoRecenteDiagrama($diagrama, 'edicao_diagrama', 'Criação do diagrama ' . $diagrama->nome);
            limpar_cache();
        });
...
<bpmn:definitions xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:bpmn=\"http://www.omg.org/spec/BPMN/20100524/MODEL\" xmlns:bpmndi=\"http://www.omg.org/spec/BPMN/20100524/DI\" xmlns:dc=\"http://www.omg.org/spec/DD/20100524/DC\" id=\"Definitions_141dzwv\" targetNamespace=\"http://bpmn.io/schema/bpmn\">
  ...
  <bpmndi:BPMNDiagram id=\"BPMNDiagram_1\">
    ...
    public function EvalidadorDoDiagrama($codusuario)
    {
        if (empty($this->validador))
            return false;
        $validador = $this->validador;
        return $validador->codusuario == $codusuario;
    }
     function eValidador(){
            return false;
     }
